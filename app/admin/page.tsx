...
export default function AdminPage() {
  ...
  /* ── Shared input class ── */
  const inp = 'input-dark w-full border rounded-lg px-3 py-2 text-sm';
  const inpSm = 'input-dark border rounded-lg px-3 py-1.5 text-sm';
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2.5">
            <svg className="w-5 h-5 text-terra" viewBox="0 0 24 24" fill="none"
            ...
            <h1 className="text-xl font-bold text-brown">פלורנטין — ניהול</h1>
          </div>
          <div className="flex items-center gap-3">
            <a href="/" className="text-sm text-terra hover:text-terra-light transition-colors">
              ← חזרה לאתר
            </a>
            <button onClick={handleLogout}
              className="text-sm text-brown-mid hover:text-terra border border-border
                hover:border-terra/40 px-3 py-1.5 rounded-lg transition-colors">
              יציאה
            </button>
          </div>
        </div>
            ...
            <button key={t} onClick={() => handleTabChange(t)}
              className={`pb-2.5 px-4 text-sm font-medium border-b-2 transition-colors
                ${tab === t
                  ...
                  : 'border-transparent text-brown-mid hover:text-brown'}`}>
              {t === 'appointments' ? 'תורים' : t === 'settings' ? 'הגדרות' : 'שירותים'}
            </button>
          ))}
        </div>
          ...
          <div>
            ...
            <div className="flex gap-2 mb-5 flex-wrap items-center justify-between">
                  ...
                  <button key={val} onClick={() => setApptFilter(val)}
                    className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors
                      ${apptFilter === val
                        ...
                        : 'bg-surface-2 text-brown-mid border-border hover:border-terra/40'}`}>
                    {label}
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors
                  ${creatingAppt
                    ? 'bg-surface-2 text-brown-mid border-border'
            ...
            {creatingAppt && (
              <div className="bg-sand border border-border/60 rounded-xl p-5 mb-6">
                <h3 className="font-semibold text-brown mb-4">יצירת תור ידנית</h3>
                <div className="grid gap-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-medium text-brown-mid mb-1">שם לקוח</label>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-brown-mid mb-1">טלפון</label>
                      <input type="tel" placeholder="0501234567"
                        ...
                        className={`${inp} ${createPhoneError ? 'border-red-500/70' : ''}`} />
                      {createPhoneError && <p className="text-red-400 text-xs mt-1">{createPhoneError}</p>}
                    </div>
                  </div>

                  <div>
                    <label className="block text-xs font-medium text-brown-mid mb-1">שירות</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-brown-mid mb-1">תאריך</label>
                    <input type="date"
                  ...
                  {createForm.date && createForm.serviceId > 0 && (
                    <div>
                      <label className="block text-xs font-medium text-brown-mid mb-1">שעה</label>
                      {!createAvailability ? (
                        <p className="text-brown-light text-sm">טוען...</p>
                        ...
                        <p className="text-red-400 text-sm">אין שעות פנויות בתאריך זה</p>
                      ) : (
                            ...
                            <button key={slot} onClick={() => setCreateForm(p => ({ ...p, time: slot }))}
                              className={`py-2 rounded-lg border text-sm font-medium transition-colors
                                ${createForm.time === slot
                                  ...
                                  : 'border-border text-brown-mid hover:border-terra/50 hover:text-brown'}`}>
                              {slot}
                            </button>
                          ))}
                        </div>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-brown-mid mb-1">הערה פנימית (אופציונלי)</label>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setCreatingAppt(false); setCreateForm(emptyForm()); }}
                      className="flex-1 py-2 border border-border rounded-lg text-sm text-brown-mid
                    ...
                    <button
                      ...
                      className="btn-crimson flex-1 py-2 text-white rounded-lg text-sm font-medium disabled:opacity-40">
            ...
            {apptLoading && <p className="text-brown-light">טוען...</p>}
            {!apptLoading && filteredAppointments.length === 0 && (
              <p className="text-brown-light text-center py-16">אין תורים בקטגוריה זו</p>
            )}
            ...
            {Object.entries(byDate)
              ...
              .map(([date, appts]) => (
                <div key={date} className="mb-8">
                  <h2 className="text-sm font-semibold text-brown-mid mb-3 border-b border-border pb-2 flex items-center gap-2">
                    {new Date(date + 'T12:00:00').toLocaleDateString('he-IL', {
                      weekday: 'long', year: 'numeric', month: 'long', day: 'numeric',
                    })}
                    {date === todayStr && (
                      <span className="text-xs bg-terra/15 text-terra px-2 py-0.5 rounded-full border border-terra/25">
                        היום
                      </span>
                    )}
                  </h2>
                      ...
                      return (
                        <div key={a.id}
                          className={`border rounded-xl px-4 py-3 transition-all
                            ${isNext
                              ? 'border-terra/50 ring-1 ring-terra/20 bg-surface-2'
                              : isCancelled
                              ? 'bg-sand border-border/40 opacity-60'
                              : isCompleted
                          ...
                          {isNext && (
                            <div className="mb-2">
                              <span className="inline-flex items-center gap-1 text-xs font-semibold
                                text-terra bg-terra/12 border border-terra/25 px-2 py-0.5 rounded-full">
                                הבא בתור
                              </span>
                            </div>
                          )}
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 min-w-0">
                              <span className={`font-bold text-base shrink-0
                                ${isCancelled ? 'text-brown-light' : isCompleted ? 'text-brown-mid' : 'text-terra-light'}`}>
                                {a.time}
                              </span>
                              <span className={`font-medium text-brown ${isCancelled ? 'line-through text-brown-light' : ''}`}>
                                {a.name}
                              </span>
                              <span className="text-brown-light text-sm">{a.phone}</span>
                              <span className="text-brown-mid text-sm">{a.service}</span>
                              {isCompleted && (
                                <span className="text-xs bg-surface-2 text-brown-mid px-2 py-0.5 rounded-full border border-border/60">
                                  הושלם
                                </span>
                              )}
                              {isCancelled && (
                                <span className="text-xs bg-terra/10 text-terra/80 px-2 py-0.5 rounded-full border border-terra/20">
                                ...
                                <button onClick={() => copyAppointmentLink(a.token, a.id)}
                                  ...
                                  className={`text-xs px-2.5 py-1 rounded-lg border transition-colors
                                    ${copiedId === a.id
                                      ...
                                      : 'border-border text-brown-light hover:text-terra hover:border-terra/40'}`}>
                                  {copiedId === a.id ? '✓ הועתק' : '🔗'}
                                </button>
                              )}
                              {isConfirmed && (
                                <>
                                  <button
                                    ...
                                    className={`text-xs px-2.5 py-1 rounded-lg border transition-colors
                                      ${isEditing
                                        ? 'border-border text-brown-mid bg-surface-2'
                                        : 'border-terra/30 text-terra hover:bg-terra/10'}`}>
                                    {isEditing ? 'סגור' : 'עריכה'}
                                  </button>
                                  <button onClick={() => handleComplete(a.id)}
                                    className="text-xs px-2.5 py-1 rounded-lg border border-green-500/30
                                      text-green-400 hover:bg-green-500/10 transition-colors">
                                    הושלם ✓
                                  </button>
                                  <button onClick={() => { setCancellingId(a.id); setCancelReason(''); }}
                                    className="text-xs px-2.5 py-1 rounded-lg border border-red-500/30
                          ...
                          {a.status === 'cancelled_by_admin' && a.cancel_reason && (
                            <p className="mt-1.5 text-xs text-terra/70">סיבה: {a.cancel_reason}</p>
                          )}

                          {/* ── Inline edit form ── */}
                          {isEditing && (
                            <div className="mt-3 bg-surface-2 border border-terra/20 rounded-lg p-4">
                              <p className="text-sm font-semibold text-brown mb-3">עריכת תור</p>
                              <div className="grid gap-3">
                                <div className="grid grid-cols-2 gap-3">
                                  <div>
                                    <label className="block text-xs font-medium text-brown-mid mb-1">שם</label>
                                    <input type="text"
                                      value={editForm.name}
                                  ...
                                  <div>
                                    ...
                                    <input type="tel"
                                      ...
                                      className={`${inpSm} w-full ${editPhoneError ? 'border-red-500/70' : ''}`} />
                                    {editPhoneError && <p className="text-red-400 text-xs mt-1">{editPhoneError}</p>}
                                  </div>
                                </div>
                                ...
                                {editForm.date && editForm.serviceId > 0 && (
                                  <div>
                                    ...
                                    {!editAvailability ? (
                                      ...
                                      <p className="text-red-400 text-sm">אין שעות פנויות</p>
                                    ) : (
                                      <div className="grid grid-cols-4 gap-1.5">
                                        {editSlots.map(slot => (
                                ...
                                <div>
                                  <label className="block text-xs font-medium text-brown-mid mb-1">הערה פנימית</label>
                                  <input type="text" placeholder="הערה..."
                                    value={editForm.note}
                                ...
                                <div className="flex gap-2">
                                  <button onClick={() => setEditingApptId(null)}
                                    className="flex-1 py-2 border border-border rounded-lg text-sm
                                      text-brown-mid hover:bg-sand transition-colors">
                                    ביטול
                                  </button>
                                  <button
                          ...
                          {isCancelling && (
                            <div className="mt-3 rounded-lg p-3 border"
                              style={{ background: 'rgba(30,23,18,0.95)', borderColor: 'rgba(149,18,44,0.30)' }}>
                              <p className="text-sm font-medium text-brown mb-2">ביטול התור של {a.name}</p>
                              <input
                                type="text"
                                placeholder="סיבת הביטול (אופציונלי)"
                                value={cancelReason}
                                onChange={e => setCancelReason(e.target.value)}
                                className={`input-dark w-full text-sm border rounded-lg px-3 py-1.5 mb-2`}
                              />
                              <div className="flex gap-2">
                                <button onClick={() => setCancellingId(null)}
                                  className="flex-1 py-1.5 text-sm border border-border rounded-lg
                                    text-brown-mid hover:bg-surface-2 transition-colors">
                                  חזרה
                                </button>
                                <button onClick={() => confirmCancel(a.id)}
                                  className="btn-crimson flex-1 py-1.5 text-sm text-white rounded-lg font-medium">
                                  אשר ביטול
                                </button>
                              </div>
                            </div>
                              ...
                              {isEditingNote ? (
                                <div className="flex gap-2 items-start">
                                  <textarea
                                    ...
                                    rows={2}
                                    className="input-dark flex-1 text-sm border rounded-lg px-3 py-1.5 resize-none"
                                  />
                                  <div className="flex flex-col gap-1">
                                    <button onClick={() => saveNote(a.id)}
                                      className="btn-crimson text-xs px-3 py-1 text-white rounded-lg">
                                      שמור
                                    </button>
                                ...
                                <button
                                  ...
                                  className="text-xs text-brown-light hover:text-terra transition-colors">
                                  {a.note
                                    ? <span className="text-brown-mid">✎ {a.note}</span>
                                    : '+ הוסף הערה'}
                                </button>
                              )}
                            </div>
        ...
        {tab === 'services' && (
          <div className="grid gap-6">
            {serviceMsg && (
              <div className="bg-terra/15 border border-terra/30 text-terra px-4 py-2 rounded-lg
            ...
            {/* Add new service */}
            <section className="bg-sand border border-border rounded-xl p-5">
              <h2 className="font-semibold text-brown text-lg mb-4">הוסף שירות</h2>
              <div className="flex flex-wrap gap-2">
                <input type="text" placeholder="שם השירות"
                  value={newService.name}
                  onChange={e => setNewService(p => ({ ...p, name: e.target.value }))}
                  className="input-dark flex-1 min-w-32 border rounded-lg px-3 py-2 text-sm" />
                <input type="number" placeholder="משך (דק׳)" value={newService.duration} min={5}
                  ...
                  className="input-dark w-28 border rounded-lg px-3 py-2 text-sm" />
                <input type="number" placeholder="מחיר ₪ (אופציונלי)" value={newService.price}
                  onChange={e => setNewService(p => ({ ...p, price: e.target.value }))}
                  className="input-dark w-36 border rounded-lg px-3 py-2 text-sm" />
                <button
                  ...
                  className="btn-crimson px-4 py-2 text-white rounded-lg text-sm font-medium disabled:opacity-40">
                  הוסף
                </button>
              </div>
            </section>
            ...
            {/* Services list */}
            <section className="bg-sand border border-border rounded-xl p-5">
              <h2 className="font-semibold text-brown text-lg mb-4">רשימת שירותים</h2>
              {services.length === 0 && <p className="text-brown-light text-sm">אין שירותים</p>}
              <div className="grid gap-3">
                {services.map(s => (
                  <div key={s.id}
                    className={`border rounded-lg p-4 transition-colors
                      ${s.active ? 'border-border bg-surface-2' : 'border-border/40 bg-surface-2 opacity-50'}`}>
                    {editingService?.id === s.id ? (
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-2 bg-green-700/80 hover:bg-green-700 text-white rounded-lg text-sm font-medium transition-colors">
                          שמור
                        </button>
                        <button onClick={() => setEditingService(null)}
                          className="px-3 py-2 border border-border rounded-lg text-sm text-brown-mid
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <span className="font-medium text-brown">{s.name}</span>
                          <span className="text-brown-light text-sm mr-3">{s.duration} דק׳</span>
                          {s.price != null && <span className="text-brown-mid text-sm">· ₪{s.price}</span>}
                          {!s.active && (
                            <span className="text-xs bg-surface-2 text-brown-light px-2 py-0.5
                              rounded-full border border-border mr-2">לא פעיל</span>
                          )}
                        </div>
                        <div className="flex gap-2">
                          <button
                          ...
                          <button onClick={() => setEditingService({ ...s })}
                            className="text-sm px-3 py-1 rounded-lg border border-border text-brown-mid
                              hover:bg-surface-2 transition-colors">
                            עריכה
                          </button>
                        </div>
        ...
        {tab === 'settings' && (
          <div className="grid gap-6">
            ...
            {/* Working hours */}
            <section className="bg-sand border border-border rounded-xl p-5">
              <h2 className="font-semibold text-brown text-lg mb-4">שעות עבודה</h2>
              <div className="flex items-center gap-4 flex-wrap">
                <label className="flex items-center gap-2">
                  <span className="text-sm font-medium text-brown-mid">פתיחה</span>
                  <select value={settings.open_hour}
                    onChange={e => setSettings(p => ({ ...p, open_hour: Number(e.target.value) }))}
                    className="input-dark border rounded-lg px-3 py-2">
                ...
                <label className="flex items-center gap-2">
                  <span className="text-sm font-medium text-brown-mid">סגירה</span>
                ...
                <button onClick={saveWorkingHours}
                  className="btn-crimson px-4 py-2 text-white rounded-lg text-sm font-medium">
                  שמור
                </button>
              </div>
            </section>
            ...
            {/* Working days */}
            <section className="bg-sand border border-border rounded-xl p-5">
              <h2 className="font-semibold text-brown text-lg mb-4">ימי עבודה</h2>
                  ...
                  <button key={idx} onClick={() => toggleDay(idx)}
                    className={`px-3 py-1.5 rounded-lg border-2 text-sm font-medium transition-colors
                      ${settings.working_days.includes(idx)
                        ? 'border-terra bg-terra text-white'
                        : 'border-border text-brown-mid hover:border-terra/50'}`}>
                    {name}
                  </button>
                ))}
              </div>
            ...
            {/* Days off */}
            <section className="bg-sand border border-border rounded-xl p-5">
              <h2 className="font-semibold text-brown text-lg mb-4">ימי חופש / חגים</h2>
              <div className="flex gap-2 mb-4">
                <input type="date" value={newDayOff}
                  onChange={e => setNewDayOff(e.target.value)}
                  className="input-dark flex-1 border rounded-lg px-3 py-2 text-sm" />
              ...
              {daysOff.length === 0
                ? <p className="text-brown-light text-sm">אין ימי חופש מוגדרים</p>
                : daysOff.map(d => (
                    <div key={d} className="flex items-center justify-between py-2 border-b border-border/40 last:border-0">
                      <span className="text-sm text-brown">{new Date(d + 'T12:00:00').toLocaleDateString('he-IL', {
                        weekday: 'long', year: 'numeric', month: 'long', day: 'numeric',
                      })}</span>
                      <button onClick={() => removeDayOff(d)}
                        className="text-red-400/70 hover:text-red-400 text-sm px-2 py-1
            ...
            {/* Blocked time ranges */}
            <section className="bg-sand border border-border rounded-xl p-5">
              <h2 className="font-semibold text-brown text-lg mb-1">טווחי זמן חסומים</h2>
              <p className="text-brown-light text-xs mb-4">ללא תאריך = חוזר כל יום עבודה</p>
              <div className="flex flex-wrap gap-2 mb-4">
                <input type="time" value={newRange.start_time}
                  onChange={e => setNewRange(p => ({ ...p, start_time: e.target.value }))}
                  className="input-dark border rounded-lg px-3 py-2 text-sm" />
                <span className="flex items-center text-brown-mid">עד</span>
            ...
            {/* Change password */}
            <section className="bg-sand border border-border rounded-xl p-5">
              <h2 className="font-semibold text-brown text-lg mb-4">שינוי סיסמה</h2>
              <div className="grid gap-3 max-w-sm">
                <div>
                  <label className="block text-sm font-medium text-brown-mid mb-1">סיסמה נוכחית</label>
                  <input type="password" value={pwForm.current}
                    onChange={e => setPwForm(p => ({ ...p, current: e.target.value }))}
                    className={inp} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-brown-mid mb-1">סיסמה חדשה</label>
                  <input type="password" value={pwForm.next}
                    onChange={e => setPwForm(p => ({ ...p, next: e.target.value }))}
                    className={inp} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-brown-mid mb-1">אימות סיסמה חדשה</label>
